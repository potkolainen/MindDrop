...
function Settings({ searchEngine, setSearchEngine }) {
  ...
  const [enabledSearchEngines, setEnabledSearchEngines] = useState(() => {
    const saved = localStorage.getItem('enabled-search-engines');
    return saved ? JSON.parse(saved) : ['duckduckgo', 'brave', 'bing'];
  });
  const [collapsedSections, setCollapsedSections] = useState(() => {
    const saved = localStorage.getItem('settings-collapsed-sections');
    return saved ? JSON.parse(saved) : ['tts', 'api', 'download', 'search']; // All collapsed by default
  });
  ...
  useEffect(() => {
    localStorage.setItem('enabled-search-engines', JSON.stringify(enabledSearchEngines));
  }, [enabledSearchEngines]);

  useEffect(() => {
    localStorage.setItem('settings-collapsed-sections', JSON.stringify(collapsedSections));
  }, [collapsedSections]);
  ...
  const toggleSearchEngine = (engineId) => {
  ...
  return (
    <div className="settings">
      ...
      {/* Python Environment - Always visible, non-collapsible */}
      ...
      {/* Memory Management - Always visible, non-collapsible */}
      <div className="settings-section">
      ...
      {/* Text-to-Speech - Collapsible */}
      <div className="settings-section">
        <h3 
          onClick={() => toggleSection('tts')}
          style={{ cursor: 'pointer', userSelect: 'none' }}
        >
          Text-to-Speech {collapsedSections.includes('tts') ? '▶' : '▼'}
        </h3>
        {!collapsedSections.includes('tts') && (
          <div className="info-box">
            <p className="info-text hint" style={{ marginBottom: '12px' }}>
      ...
      {/* Download Directory - Collapsible */}
      <div className="settings-section">
        <h3 
          onClick={() => toggleSection('download')}
          style={{ cursor: 'pointer', userSelect: 'none' }}
        >
          Download Directory {collapsedSections.includes('download') ? '▶' : '▼'}
        </h3>
        {!collapsedSections.includes('download') && (
          <div className="info-box">
            <p className="info-text hint" style={{ marginBottom: '12px' }}>
              Current download location for models
            </p>
            <div className="download-dir-display">
              <code>{downloadDir || 'Loading...'}</code>
            </div>
            <button
            ...
            >
              {isChangingDir ? 'Changing...' : '📁 Change Directory'}
            </button>
          </div>
        )}
      </div>

      {/* API Authentication - Collapsible */}
      <div className="settings-section">
        <h3 
          onClick={() => toggleSection('api')}
          style={{ cursor: 'pointer', userSelect: 'none' }}
        >
          API Authentication {collapsedSections.includes('api') ? '▶' : '▼'}
        </h3>
        {!collapsedSections.includes('api') && (
          <div className="info-box">
            <p className="info-text hint" style={{ marginBottom: '12px' }}>
      ...
      {/* Search Engine - Collapsible */}
      <div className="settings-section">
        <h3 
          onClick={() => toggleSection('search')}
          style={{ cursor: 'pointer', userSelect: 'none' }}
        >
          Search Engine {collapsedSections.includes('search') ? '▶' : '▼'}
        </h3>
        {!collapsedSections.includes('search') && (
          <div className="info-box">
            <p className="info-text hint" style={{ marginBottom: '12px' }}>
            ...
            <div className="setting-item">
              ...
              >
                {SEARCH_ENGINES.filter(e => e.id !== 'multi-engine').map(engine => (
            ...
            <p className="info-text hint" style={{ marginTop: '16px', marginBottom: '8px' }}>
              Select engines for Multi-Engine search (min 1 required):
            </p>
          <div className="search-engine-grid">
            {SEARCH_ENGINES.map(engine => (
              <button
                key={`multi-${engine.id}`}
                className={`search-engine-option ${enabledSearchEngines.includes(engine.id) ? 'enabled' : 'disabled'}`}
                onClick={() => toggleSearchEngine(engine.id)}
                style={{ opacity: enabledSearchEngines.includes(engine.id) ? 1 : 0.4 }}
              >
                <span className="engine-icon">{engine.icon}</span>
                <span className="engine-name">{engine.name}</span>
                {enabledSearchEngines.includes(engine.id) && <span className="check-mark">✓</span>}
              </button>
            ))}
          </div>
        </div>
      ...
      {/* About - Always visible, non-collapsible, at bottom */}
      <div className="settings-section">
        <h3>About</h3>
        <div className="info-box">
          ...
          <p className="info-text" style={{ marginTop: '16px', fontWeight: '500' }}>
            Features:
          </p>
          <ul className="info-text hint" style={{ marginLeft: '20px', marginTop: '4px' }}>
            <li>Supports text, image, video, and TTS</li>
            <li>Plugin system to extend functionality</li>
            <li>Offline-first, cross-platform</li>
            <li>Persistent memory & conversation logs</li>
          </ul>
          
          <p className="info-text" style={{ marginTop: '12px', fontWeight: '500' }}>
            Technical:
          </p>
          <ul className="info-text hint" style={{ marginLeft: '20px', marginTop: '4px' }}>
            <li>Rust + React + Tauri for speed and safety</li>
            <li>Low VRAM modes for older GPUs</li>
            <li>Out-of-process plugin execution</li>
          </ul>
          
          <p className="info-text hint" style={{ marginTop: '16px' }}>
            <strong>Version:</strong> alpha 0.5.0<br/>
            <strong>License:</strong> Unlicenced<br/>
            <strong>GitHub:</strong> <a href="https://github.com/potkolainen/MindDrop" target="_blank" rel="noopener noreferrer" style={{ color: '#4a9eff' }}>https://github.com/potkolainen/MindDrop</a>
          </p>
        </div>
      </div>
    </div>
