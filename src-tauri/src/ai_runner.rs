...
pub fn run_python_inference(
    ...
    eprintln!("🚀 Starting inference for model: {}", model_id);
    eprintln!("   Quantization requested: {}", quantization);
    ...
    eprintln!("   Model cache path: {}", cache_dir_str);
...
try:
    ...
    try:
        ...
        print(f"🔧 Initial quant_type requested: {{quant_type}}", file=sys.stderr)
        
        # Check if model is already quantized by inspecting config
        # If so, we should not apply additional quantization
        is_pre_quantized = False
        native_quant_type = None
        try:
            from transformers import AutoConfig
            print(f"📦 Loading model config from: {{model_id}}", file=sys.stderr)
            try:
                config = AutoConfig.from_pretrained(
                    model_id, 
                    cache_dir=cache_dir, 
                    trust_remote_code=True,
                    local_files_only=True  # Don't try to download, use what we have
                )
                print(f"✓ Config loaded successfully", file=sys.stderr)
            except Exception as config_err:
                print(f"⚠️  Failed to load config with local_files_only, trying without: {{config_err}}", file=sys.stderr)
                config = AutoConfig.from_pretrained(model_id, cache_dir=cache_dir, trust_remote_code=True)
                print(f"✓ Config loaded successfully (fallback)", file=sys.stderr)
            
            # Check for pre-quantization
            if hasattr(config, 'quantization_config') and config.quantization_config is not None:
                quant_config = config.quantization_config
                quant_config_type = type(quant_config).__name__
                print(f"✓ Found quantization_config: {{quant_config_type}}", file=sys.stderr)
                
                # Determine native quantization type
                if 'mxfp4' in quant_config_type.lower() or 'mx4' in quant_config_type.lower():
                    native_quant_type = "4bit-mx"
                elif 'mxfp8' in quant_config_type.lower() or 'mx8' in quant_config_type.lower():
                    native_quant_type = "8bit-mx"
                elif '4bit' in quant_config_type.lower() or 'nf4' in quant_config_type.lower():
                    native_quant_type = "4bit"
                elif '8bit' in quant_config_type.lower() or 'int8' in quant_config_type.lower():
                    native_quant_type = "8bit"
                else:
                    native_quant_type = f"custom-{{quant_config_type}}"
                
                is_pre_quantized = True
                print(f"✓ Model is pre-quantized with: {{native_quant_type}}", file=sys.stderr)
                print(f"  Original request: {{quant_type}} → Forcing to 'native' to use {{native_quant_type}}", file=sys.stderr)
                quant_type = "native"
                print(f"  ✓ quant_type now: {{quant_type}}", file=sys.stderr)
            else:
                print(f"✓ No quantization_config found - model is unquantized (fp16/fp32)", file=sys.stderr)
                native_quant_type = "fp16"
        except Exception as e:
            import traceback
            print(f"❌ Exception while checking quantization config:", file=sys.stderr)
            print(f"   Type: {{type(e).__name__}}", file=sys.stderr)
            print(f"   Message: {{str(e)[:300]}}", file=sys.stderr)
            traceback.print_exc(file=sys.stderr)
            print(f"  Assuming not pre-quantized and will try user's selection", file=sys.stderr)
        
        # Master try/except to catch ANY loading error and handle pre-quantized models
        try:
            if use_gpu:
                if is_pre_quantized:
                    # Model is already quantized - load without ANY quantization-related flags
                    print("Loading pre-quantized model with native settings...", file=sys.stderr)
                    try:
                        ...
                        print("✓ Pre-quantized model loaded successfully", file=sys.stderr)
                    except Exception as load_error:
                        # If loading pre-quantized model fails, try without device_map
                        print(f"Failed with device_map, trying without: {{load_error}}", file=sys.stderr)
                        model = AutoModelForCausalLM.from_pretrained(
                            model_id,
                            cache_dir=cache_dir,
                            low_cpu_mem_usage=True,
                ...
                try:
                    ...
                    if "bitsandbytes" in error_msg.lower() or "No package metadata" in error_msg:
                        print(f"\\n❌ ERROR: bitsandbytes package is not installed.", file=sys.stderr)
                        print(f"\\nTo use 4-bit or 8-bit quantization, install bitsandbytes:", file=sys.stderr)
                        print(f"  pip install bitsandbytes accelerate", file=sys.stderr)
                        print(f"\\nFalling back to native (unquantized) mode...\\n", file=sys.stderr)
                    else:
                    ...
                    print(f"DEBUG: Caught exception during 4bit load: {{error_msg[:200]}}", file=sys.stderr)
                    if "quantization_config" in error_msg.lower() or "mxfp" in error_msg.lower() or "bitsandbytes" in error_msg.lower():
                        print(f"\\n⚠️  Model is pre-quantized - bitsandbytes quantization not compatible.", file=sys.stderr)
                        print(f"   Reloading with model's native quantization...", file=sys.stderr)
                        # Load without ANY quantization-related parameters for pre-quantized models
                        try:
                            model = AutoModelForCausalLM.from_pretrained(
                                model_id,
                                cache_dir=cache_dir,
                            ...
                            print("✓ Successfully loaded pre-quantized model", file=sys.stderr)
                        except Exception as reload_err:
                            print(f"Reload failed with device_map, trying minimal config: {{str(reload_err)[:150]}}", file=sys.stderr)
                            # Last resort: minimal loading params
                            try:
                                model = AutoModelForCausalLM.from_pretrained(
                                    model_id,
                                    cache_dir=cache_dir,
                                ...
                                print("✓ Loaded with minimal config", file=sys.stderr)
                            except Exception as final_err:
                                print(f"❌ All loading attempts failed: {{str(final_err)[:150]}}", file=sys.stderr)
                                raise
                    else:
                        print(f"4-bit quantization failed (non-pre-quant): {{quant_error}}", file=sys.stderr)
                        ...
                        try:
                            ...
                            print(f"Native fallback failed: {{str(fallback_err)[:150]}}", file=sys.stderr)
                            print("Trying without device_map...", file=sys.stderr)
                            model = AutoModelForCausalLM.from_pretrained(
                                model_id,
                                cache_dir=cache_dir,
                                trust_remote_code=True
                ...
                try:
                    ...
                    error_msg = str(quant_error)
                    print(f"DEBUG: Caught exception during 8bit load: {{error_msg[:200]}}", file=sys.stderr)
                    if "quantization_config" in error_msg.lower() or "mxfp" in error_msg.lower() or "bitsandbytes" in error_msg.lower():
                        print(f"\\n⚠️  Model is pre-quantized - bitsandbytes quantization not compatible.", file=sys.stderr)
                        print(f"   Reloading with model's native quantization...", file=sys.stderr)
                        # Load without any quantization flags for pre-quantized models
                        try:
                            model = AutoModelForCausalLM.from_pretrained(
                                model_id,
                                cache_dir=cache_dir,
                            ...
                            print(f"Reload failed, trying minimal config: {{str(reload_err)[:150]}}", file=sys.stderr)
                            model = AutoModelForCausalLM.from_pretrained(
                                model_id,
                                cache_dir=cache_dir,
                                trust_remote_code=True
                        ...
                        print(f"8-bit quantization failed (non-pre-quant): {{quant_error}}", file=sys.stderr)
                        print("Falling back to native...", file=sys.stderr)
                        try:
                            model = AutoModelForCausalLM.from_pretrained(
                                model_id,
            ...
            # Catch the specific BitsAndBytes config mismatch error
            error_str = str(ve)
            print(f"\\n🔍 Caught ValueError during loading: {{error_str[:250]}}", file=sys.stderr)
            if "quantization_config" in error_str.lower() or "bitsandbytes" in error_str.lower() or "mxfp" in error_str.lower():
                print(f"\\n⚠️  This model is pre-quantized and incompatible with bitsandbytes.", file=sys.stderr)
                print(f"   Loading with model's built-in quantization...", file=sys.stderr)
                model = AutoModelForCausalLM.from_pretrained(
                    model_id,
                    cache_dir=cache_dir,
                    trust_remote_code=True,
                ...
                print("✓ Loaded pre-quantized model successfully", file=sys.stderr)
            else:
                # Re-raise if it's a different ValueError
                raise
        except Exception as general_error:
            # Catch ANY other exception and check if it's a pre-quantization issue
            error_str = str(general_error)
            error_type = type(general_error).__name__
            print(f"\\n🔍 Caught {{error_type}} during loading: {{error_str[:250]}}", file=sys.stderr)
            if "quantization_config" in error_str.lower() or "bitsandbytes" in error_str.lower() or "mxfp" in error_str.lower():
                print(f"\\n⚠️  Model has pre-quantization that conflicts with requested quantization.", file=sys.stderr)
                print(f"   Loading with minimal config to use model's native quantization...", file=sys.stderr)
                try:
                    model = AutoModelForCausalLM.from_pretrained(
                        model_id,
                        cache_dir=cache_dir,
                    ...
                    print("✓ Successfully loaded with native quantization", file=sys.stderr)
                except Exception as final_error:
                    print(f"❌ Final attempt failed: {{str(final_error)[:200]}}", file=sys.stderr)
                    raise
            else:
                # Re-raise if it's not a quantization-related error
                print(f"Error is not quantization-related, re-raising...", file=sys.stderr)
                raise
...
try:
    # GPU availability check and device_map setup
    if torch.cuda.is_available() and not force_cpu:
        free_memory = torch.cuda.mem_get_info()[0] / (1024 ** 3)
        total_memory = torch.cuda.mem_get_info()[1] / (1024 ** 3)
        print(f"GPU memory: {{free_memory:.2f}} GB free / {{total_memory:.2f}} GB total", file=sys.stderr)
        ...
        if free_memory > gpu_threshold:
            ...
            # Handle Hybrid mode with specific GPU layer count
            if exec_mode == "hybrid" and gpu_layers is not None:
                device_map = "auto"
                # Allocate memory per layer: gpu_layers * 500MB for GPU, rest on CPU
                max_memory = {{0: f"{{gpu_layers * 500}}MiB", "cpu": "80GiB"}}
                print(f"Execution mode: Hybrid ({{gpu_layers}} layers on GPU, max {{gpu_layers * 500}}MiB)", file=sys.stderr)
            else:
                device_map = "auto"
                max_memory = None
                print(f"Execution mode: GPU", file=sys.stderr)
        else:
            device = "cpu"
            device_map = "cpu"
            max_memory = None
            use_gpu = False
            print(f"Execution mode: CPU (insufficient GPU memory)", file=sys.stderr)
    else:
        device = "cpu"
        device_map = "cpu"
        max_memory = None
        use_gpu = False
        print(f"Execution mode: CPU (forced or no CUDA)", file=sys.stderr)
    
    # Check if model is pre-quantized BEFORE loading
    print("🔧 Checking for pre-quantization...", file=sys.stderr)
    try:
        from transformers import AutoConfig
        config = AutoConfig.from_pretrained(model_id, cache_dir=cache_dir, trust_remote_code=True, local_files_only=True)
        if hasattr(config, 'quantization_config') and config.quantization_config is not None:
            quant_config_type = type(config.quantization_config).__name__
            print(f"✓ Model is PRE-QUANTIZED with {{quant_config_type}}", file=sys.stderr)
            print(f"  Ignoring requested '{{quant_type}}' and using 'native' instead", file=sys.stderr)
            quant_type = "native"
    except Exception as check_err:
        print(f"⚠️  Could not check pre-quantization: {{check_err}}", file=sys.stderr)
    
    print(f"📦 Loading model with quant_type={{quant_type}}, device_map={{device_map}}", file=sys.stderr)
    if max_memory:
        print(f"   max_memory={{max_memory}}", file=sys.stderr)
    
    # Load model with quantization (with fallback on error)
    model = None
    try:
        if use_gpu:
            if quant_type == "4bit":
        ...
        if "BitsAndBytesConfig" in str(e) or "quantization_config" in str(e):
            print(f"⚠️  Quantization conflict: {{e}}", file=sys.stderr)
            print("🔄 Retrying with native quantization...", file=sys.stderr)
            model = AutoModelForCausalLM.from_pretrained(
                model_id, cache_dir=cache_dir, torch_dtype="auto",
                device_map=device_map, max_memory=max_memory,
                low_cpu_mem_usage=True, trust_remote_code=True
